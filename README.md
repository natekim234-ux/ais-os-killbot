...
| Rule | Trigger | Why |
|---|---|---|
| **1. CPC mechanical** | `spend ≥ $15 AND (link_clicks = 0 OR CPC > $2.50)` — CPC = cost per link click, same metric as the Ads Manager CPC column | A new adset is a new test. If click economics are broken once $15 is spent, kill fast. The zero-click branch is critical: a dead ad has 0 link clicks, so cost-per-link-click is null and the CPC test silently skips it — the worst ads were the most protected (CT41 hit $53 with 0 clicks). Adset-level so a bad test can't hide inside a healthy CBO's averaged CPC. |
...
Breakeven CPP pulls live from KPI sheet G2 ($35.31 today). Rule 1's $15/$2.50 are fixed click economics.
